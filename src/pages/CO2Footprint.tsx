...
Thought for 5s
تمام، سبب إن Manual Mode واقف إننا بعد تعديل الكود خلّينا emissionData يتولد دائمًا من stageData، فالتعديل اليدوي على emissionData ممكن يتلغى أو مايبقاش ثابت.

الحل: في Manual Mode نخلي المستخدم يعدّل جدول المراحل مباشرة، وما نخليش useEffect يعيد حسابه من stageData.
...
ثم استبدل دالة handleEmissionChange بهذه:
...
لكن مهم جدًا: عندك دلوقتي طريقتين حساب مختلفتين:

في Auto / IoT
البرنامج يحسب Carbon Cost من مجموع تفاصيل البنود عشان يطلع مثلًا 19.04 EGP.

في Manual
لما تدخل رقم الانبعاثات يدويًا، مفيش تفاصيل بنود مقابلة للرقم الجديد، لذلك التكلفة هتتحسب من الرقم اليدوي مباشرة:

emissions × carbon price × exchange rate
يعني ده طبيعي، لأن Manual Mode بيعدل مرحلة كاملة وليس item-level details.

لو عايز Manual Mode كمان يعدّل تفاصيل البنود نفسها، يبقى لازم نخلي الـ input في جدول التفاصيل، مش في الجدول الرئيسي فقط.


عدل برده فى تفلصيل البنود نفسها

Thought for 8s
تمام. عشان Manual Mode يعدّل كمان في تفاصيل البنود نفسها، خلي كل الحسابات ترجع من stageData، ولما تعدّل رقم مرحلة في الجدول الرئيسي، نوزّع الفرق على البنود التابعة لها. ولما تعدّل بند داخل التفاصيل، يتحدّث الجدول الرئيسي تلقائيًا.

اعمل التعديلات دي في الكود الذي أرسلته. 


1. اترك هذا الـ useEffect كما هو
لا تغيّره للنسخة التي قلتها قبل كده. لازم يفضل كده:

useEffect(() => {
  setEmissionData(getEmissionData());
}, [stageData, mode]);
2. استبدل handleEmissionChange بهذه النسخة
هذه تجعل تعديل المرحلة في الجدول الرئيسي ينعكس على تفاصيل البنود بنسبة proportional.
...
const handleEmissionChange = (index: number, value: string) => {
  ...
  if (isNaN(newValue) || !isFinite(newValue) || newValue < 0) return;
  ...
  setStageData((previousData) => {
    const updatedData: StageData = JSON.parse(JSON.stringify(previousData));
    const items = updatedData[category] || [];

    if (items.length === 0) return updatedData;
    ...
    if (currentTotal > 0) {
      ...
      updatedData[category] = items.map((item: any) => ({
        ...item,
        emissions: parseFloat(((item.emissions || 0) * scaleFactor).toFixed(6)),
      }));
    } else {
      updatedData[category] = items.map((item: any, itemIndex: number) => ({
        ...item,
        emissions: itemIndex === 0 ? parseFloat(newValue.toFixed(6)) : 0,
      }));
    }
...
3. أضف دالة جديدة لتعديل انبعاثات كل بند
ضعها بعد handleEmissionChange أو قبل handleEmissionFactorChange.

const handleItemEmissionChange = (stage: string, index: number, value: string) => {
  if (mode !== 'manual') return;
  ...
  setStageData((previousData) => {
    ...
    if (!updatedData[stage] || !updatedData[stage][index]) {
      return updatedData;
    }

    updatedData[stage][index] = {
      ...updatedData[stage][index],
      emissions: parseFloat(newValue.toFixed(6)),
    };

    return updatedData;
  });
};
4. عدّل خلية Emissions (kg CO₂e) داخل جدول التفاصيل
ابحث عن هذا الجزء داخل currentStageData.map:
...
<Table.Cell style={compactNumberStyle}>
  {mode === 'manual' ? (
    <TextField.Root
      size="1"
      value={(item.emissions || 0).toString()}
      onChange={(event) => handleItemEmissionChange(openStage || '', index, event.target.value)}
      style={{ maxWidth: 120, fontSize: '13px' }}
    />
  ) : (
    formatNumber(item.emissions, 3)
...
5. عدّل showItemCostDetails لتحسين عرض الدولار
استبدل هذا السطر:
...
في Manual Mode تقدر تعدّل رقم المرحلة من الجدول الرئيسي.

البرنامج يوزّع الرقم الجديد على البنود التابعة للمرحلة.

تقدر تفتح التفاصيل وتعدّل Emissions لكل بند مباشرة.

تكلفة الكربون في الجدول الرئيسي ستظل مساوية لمجموع تفاصيل البنود، مثل 19.04 EGP.

لو عدّلت بند داخل التفاصيل، رقم المرحلة في الجدول الرئيسي يتحدث تلقائيًا.


أرسل الأن الكود كاملا مكملا مصححا بالانجليزية

Thought for 51s
أكيد، هذا هو الكود كاملًا مصححًا بالإنجليزية. التعديل الأساسي هنا: Manual Mode الآن يعدّل أرقام المرحلة، ويعدّل أيضًا Emissions داخل تفاصيل البنود نفسها، وكل التكاليف في الجدول الرئيسي تُحسب من مجموع تكاليف البنود حتى تساوي التفاصيل. 
...
const initialStageData: StageData = {
  ...
  Manufacturing: [
    ...
    { process: "Material Mixing", quantity: 0.5, unit: "kWh", emissionFactor: 0.55, emissions: 0.5 * 0.55 },
    { process: "Liquid Filling", quantity: 0.3, unit: "kWh", emissionFactor: 0.55, emissions: 0.3 * 0.55 },
    { process: "Sterilization/Microbial Control", quantity: 1.5, unit: "kWh", emissionFactor: 0.55, emissions: 1.5 * 0.55 },
    { process: "Primary Packaging", quantity: 0.2, unit: "kWh", emissionFactor: 0.55, emissions: 0.2 * 0.55 },
    { process: "Quality Inspection", quantity: 0.3, unit: "kWh", emissionFactor: 0.55, emissions: 0.3 * 0.55 },
  ],
  ...
  Transport: [
    { type: "Refrigerated Storage", duration: 7, unit: "days", emissionFactor: 0.0075, emissions: (7 * 0.0075) / BATCH_SIZE },
    { type: "Local Transport", distance: 50, unit: "km", emissionFactor: 0.062, emissions: (50 * 0.062) / BATCH_SIZE },
    { type: "Long-Distance Transport", distance: 300, unit: "km", emissionFactor: 0.062, emissions: (300 * 0.062) / BATCH_SIZE },
  ],

  Distribution: [
    { activity: "Warehouse Storage", duration: 3, unit: "days", emissionFactor: 0.01, emissions: (3 * 0.01) / BATCH_SIZE },
    { activity: "Last-Mile Delivery", distance: 15, unit: "km", emissionFactor: 0.18, emissions: (15 * 0.18) / BATCH_SIZE },
  ...
  Use: [
    ...
    { aspect: "Product Refrigeration", duration: 14, unit: "days", emissionFactor: 0.00752, emissions: (14 * 0.00752) / BATCH_SIZE },
    { aspect: "Product Preparation", quantity: 0, unit: "kWh", emissionFactor: 0, emissions: 0 },
  ],
...
const calculateCarbonCost = (emissionsKg: number) => {
  const validEmissions = safeNumber(emissionsKg);
  const emissionsTon = validEmissions / KG_PER_TON;
  const costUSD = emissionsTon * CARBON_PRICE_PER_TON;
  const costEGP = costUSD * EXCHANGE_RATE;
...
const calculateStageCostFromItems = (items: any[]) => {
  const costUSD = items.reduce((sum, item) => sum + calculateItemCarbonCost(item.emissions).costUSD, 0);
  const costEGP = items.reduce((sum, item) => sum + calculateItemCarbonCost(item.emissions).costEGP, 0);
...
const SimpleCarbonCostDialog = ({ open, onOpenChange, data }: SimpleCarbonCostDialogProps) => {
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Content style={{ maxWidth: 500 }}>
        <Dialog.Title>Carbon Cost Details</Dialog.Title>
        ...
        <Box>
          ...
          <Table.Root>
            <Table.Body>
              <Table.Row>
                <Table.RowHeaderCell>Total Emissions</Table.RowHeaderCell>
                <Table.Cell>{data ? `${data.emissions.toFixed(3)} kg CO₂e` : "0.000 kg CO₂e"}</Table.Cell>
              </Table.Row>

              <Table.Row>
                <Table.RowHeaderCell>Carbon Price</Table.RowHeaderCell>
                <Table.Cell>{data ? `${data.carbonPricePerTon} USD/ton` : "0 USD/ton"}</Table.Cell>
              </Table.Row>

              <Table.Row>
                <Table.RowHeaderCell>Carbon Price (per kg)</Table.RowHeaderCell>
                <Table.Cell>{data ? `${data.carbonPricePerKg.toFixed(4)} USD/kg` : "0.0000 USD/kg"}</Table.Cell>
              </Table.Row>
              ...
              <Table.Row>
                <Table.RowHeaderCell>Cost (US Dollars)</Table.RowHeaderCell>
                <Table.Cell>
                  <Text weight="bold">{data ? `${data.costUSD} USD` : "0.0000 USD"}</Text>
                </Table.Cell>
              </Table.Row>

              <Table.Row>
                <Table.RowHeaderCell>Exchange Rate</Table.RowHeaderCell>
                <Table.Cell>{data ? `${data.exchangeRate} EGP/USD` : "0 EGP/USD"}</Table.Cell>
              </Table.Row>
...
const CO2Footprint = () => {
  ...
  const calculateStageEmissions = (items: any[]) => {
    return parseFloat(items.reduce((sum, item) => sum + safeNumber(item.emissions), 0).toFixed(3));
  };

  const getEmissionData = (): EmissionDataItem[] => {
    const categories = ["Raw Materials", "Manufacturing", "Packaging", "Transport", "Distribution", "Use", "End of Life"];
  ...
  const totalEmissions = useMemo(() => {
    return parseFloat(emissionData.reduce((sum, item) => sum + safeNumber(item.emissions), 0).toFixed(3));
  }, [emissionData]);
  ...
  const ghgScopeData = useMemo(() => {
    ...
    const scope3Emissions = emissionData
      .filter((item) => ["Raw Materials", "Transport", "Distribution", "Use", "End of Life"].includes(item.category))
      .reduce((sum, item) => sum + item.emissions, 0);
  ...
  const pieChartData = emissionData.map((item) => ({
    name: item.category,
    value: safeNumber(item.emissions),
    cost: currency === "EGP" ? safeNumber(item.costEGP) : safeNumber(item.costUSD),
  }));
  ...
  const totalStageEmissions = currentStageData.reduce((sum, item) => sum + safeNumber(item.emissions), 0);

  const totalStageCostUSD = currentStageData.reduce((sum, item) => {
    return sum + calculateItemCarbonCost(item.emissions).costUSD;
  }, 0);

  const totalStageCostEGP = currentStageData.reduce((sum, item) => {
    return sum + calculateItemCarbonCost(item.emissions).costEGP;
  }, 0);
  ...
  const showCostDetails = (item: EmissionDataItem) => {
    const emissionsKg = safeNumber(item.emissions);
  ...
  const handleEmissionChange = (index: number, value: string) => {
    ...
    setStageData((previousData) => {
      ...
      const currentTotal = items.reduce((sum, item) => {
        return sum + safeNumber(item.emissions);
      }, 0);
      ...
      if (currentTotal > 0) {
        ...
        updatedData[category] = items.map((item) => ({
          ...item,
          emissions: parseFloat((safeNumber(item.emissions) * scaleFactor).toFixed(6)),
        }));
      } else {
        updatedData[category] = items.map((item, itemIndex) => ({
          ...item,
          emissions: itemIndex === 0 ? parseFloat(newValue.toFixed(6)) : 0,
        }));
      }
  ...
  const handleEmissionFactorChange = (stage: string, index: number, value: string) => {
    ...
    setStageData((previousData) => {
      ...
      item.emissions = parseFloat(item.emissions.toFixed(6));
  ...
  const getFormattedQuantity = (item: any, stage: string) => {
    if (stage === "Manufacturing") return Number(item.quantity || 0).toFixed(2);
    if (stage === "Transport" || stage === "Distribution" || stage === "Use") {
      if (item.distance !== undefined) return `${item.distance}`;
      if (item.duration !== undefined) return `${item.duration}`;
      if (item.quantity !== undefined) return `${item.quantity}`;
  ...
  const modeDescription =
    mode === "iot"
      ? "Live simulated IoT readings are active. Quantities may change every 3 seconds."
      : mode === "manual"
        ? "Manual mode is active. Category emissions and item emissions can be edited manually."
        : "Auto mode is active. Default baseline values are used.";
  ...
  return (
    ...
    >
      ...
      <Grid columns={{ initial: "1", sm: "2", lg: "4" }} gap="4" mb="5">
        ...
        <Card style={{ borderRadius: "16px" }}>
          <Flex direction="column" gap="1" p="4">
            ...
            <Heading size="7">{isNaN(carbonIntensity) || !isFinite(carbonIntensity) ? "0.0000" : carbonIntensity.toFixed(4)}</Heading>
            <Text size="1" color="gray">
              kg CO₂e per {currency === "USD" ? "$" : "EGP"}K revenue
            </Text>
          </Flex>
        ...
        <Box p="3">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={pieChartData}>
              <XAxis dataKey="name" />
              <YAxis />
              <Tooltip formatter={(value: number) => [`${value.toFixed(currency === "EGP" ? 2 : 4)} ${currency}`, "Carbon Cost"]} />
              <Legend />
              <Bar dataKey="cost" name={`Carbon Cost (${currency})`} />
            </BarChart>
          </ResponsiveContainer>
            ...
            <Table.Root variant="surface">
              ...
              <Table.Body>
                {emissionData.map((item, index) => (
                  <Table.Row key={item.category}>
                    <Table.Cell>
                      <Button variant="ghost" onClick={() => handleStageClick(item.category)} style={{ padding: 0, fontWeight: "bold" }}>
                        {item.category}
                      </Button>
                    </Table.Cell>
                    ...
                    <Table.Cell>
                      <Button variant="ghost" onClick={() => showCostDetails(item)} style={{ padding: 0 }}>
                        {currency === "EGP" ? `${item.costEGP.toFixed(2)} EGP` : `${item.costUSD.toFixed(4)} USD`}
                      </Button>
                    </Table.Cell>

                    <Table.Cell>
                      <strong>{totalEmissions === 0 ? "0.0" : ((item.emissions / totalEmissions) * 100).toFixed(1)}%</strong>
                    </Table.Cell>
        ...
        >
          ...
          <Box style={{ overflowY: "auto", maxHeight: "62vh" }}>
            <Table.Root variant="surface">
              <Table.Header>
                <Table.Row>
                  <Table.ColumnHeaderCell style={softBlueHeaderStyle}>{getNameColumnTitle(openStage)}</Table.ColumnHeaderCell>
                  <Table.ColumnHeaderCell style={softBlueHeaderStyle}>{getQuantityColumnTitle(openStage)}</Table.ColumnHeaderCell>

                  {shouldShowUnitColumn(openStage) && <Table.ColumnHeaderCell style={softBlueHeaderStyle}>Unit</Table.ColumnHeaderCell>}

                  {openStage === "Packaging" && <Table.ColumnHeaderCell style={softBlueHeaderStyle}>Material</Table.ColumnHeaderCell>}

                  <Table.ColumnHeaderCell style={softBlueHeaderStyle}>{getEmissionFactorColumnTitle(openStage)}</Table.ColumnHeaderCell>

                  <Table.ColumnHeaderCell style={softBlueHeaderStyle}>Emissions (kg CO₂e)</Table.ColumnHeaderCell>
                  ...
                  <Table.ColumnHeaderCell style={softBlueHeaderStyle}>
                    <Flex align="center" gap="2">
                      ...
                      <Select.Root value={stageCostCurrency} onValueChange={(value) => setStageCostCurrency(value as "USD" | "EGP")}>
                        <Select.Trigger
                          variant="soft"
                          style={{
                            width: 72,
                  ...
                  <Table.ColumnHeaderCell style={softBlueHeaderStyle}>Stage %</Table.ColumnHeaderCell>
                </Table.Row>
              </Table.Header>
              ...
              <Table.Body>
                {currentStageData.map((item: any, index: number) => {
                  const itemCost = calculateItemCarbonCost(item.emissions);
                  const itemShare = totalStageEmissions > 0 ? ((item.emissions / totalStageEmissions) * 100).toFixed(1) : "0.0";
                  ...
                  return (
                    <Table.Row key={`${openStage}-${index}`}>
                      ...
                      {shouldShowUnitColumn(openStage) && <Table.Cell style={compactNumberStyle}>{item.unit}</Table.Cell>}

                      {openStage === "Packaging" && <Table.Cell style={{ fontSize: "13px" }}>{item.material}</Table.Cell>}
                      ...
                      <Table.Cell style={compactNumberStyle}>
                        <Button variant="ghost" onClick={() => showItemCostDetails(item, openStage || "")} style={compactButtonStyle}>
                          {stageCostCurrency === "USD" ? `${itemCost.costUSD.toFixed(4)} USD` : `${itemCost.costEGP.toFixed(2)} EGP`}
                        </Button>
                      </Table.Cell>

                      <Table.Cell style={compactNumberStyle}>{itemShare}%</Table.Cell>
                    </Table.Row>
                  );
                })}
                ...
                <Table.Row style={{ backgroundColor: "#EEF4FF" }}>
                  <Table.RowHeaderCell
                    colSpan={openStage === "Packaging" ? 4 : shouldShowUnitColumn(openStage) ? 4 : 3}
                    style={{ fontWeight: "bold", fontSize: "13px" }}
                  >
                    <strong>Total</strong>
                  </Table.RowHeaderCell>
                  ...
                  <Table.Cell style={compactNumberStyle}>
                    <strong>
                      {stageCostCurrency === "USD" ? `${totalStageCostUSD.toFixed(4)} USD` : `${totalStageCostEGP.toFixed(2)} EGP`}
                    </strong>
                  </Table.Cell>
      ...
      <SimpleCarbonCostDialog open={costDetailsOpen} onOpenChange={setCostDetailsOpen} data={currentCostDetails} />

      <SimpleCarbonCostDialog open={itemCostDetailsOpen} onOpenChange={setItemCostDetailsOpen} data={currentItemCostDetails} />
    </Box>
  );
};
