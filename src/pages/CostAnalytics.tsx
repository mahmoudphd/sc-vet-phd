...
const CostAnalysis = () => {
  ...
  const defaultRawMaterials = simulatedIoTCostData.rawMaterials;

  const autoRawMaterials = defaultRawMaterials.map(item => ({
    ...item,
    pricePerKg: +(item.pricePerKg * 1.1).toFixed(2),
    cost: +(item.weightKg * item.pricePerKg * 1.1).toFixed(2)
  }));

  const [rawData, setRawData] = useState(defaultRawMaterials);
  ...
  useEffect(() => {
    if (mode === 'auto') {
      ...
      setCostData(mapped);
      setRawData(autoRawMaterials);
    } else {
      setCostData(defaultCostData);
      setRawData(defaultRawMaterials);
    }
  }, [mode]);
      ...
      <Table.Root variant="surface">
        ...
        <Table.Body>
          {costData.map((item, index) => {
            const isDirectMaterials = item.category === 'Direct Materials';
            ...
            return (
              <React.Fragment key={item.category}>
                <Table.Row>
                  ...
                  <Table.Cell>
                    {isDirectMaterials ? (
                      <Button variant="outline" size="1" onClick={() => setShowRawDialog(!showRawDialog)}>
                        {showRawDialog ? 'Hide Details' : 'View Raw Materials'}
                      </Button>
                    ) : (
                      <Select.Root
                ...
                {isDirectMaterials && showRawDialog && (
                  <Table.Row>
                    <Table.Cell colSpan={7}>
                      <Table.Root variant="surface">
                        <Table.Header>
                        ...
                        <Table.Body>
                          {rawData.map((raw, idx) => (
                            <Table.Row key={idx}>
                              <Table.Cell>{raw.name}</Table.Cell>
                              <Table.Cell>{raw.weightKg}</Table.Cell>
                              <Table.Cell>{currency} {raw.pricePerKg}</Table.Cell>
                              <Table.Cell>{currency} {raw.cost}</Table.Cell>
                            </Table.Row>
                          ))}
                        </Table.Body>
                      </Table.Root>
