        ...
        <Dialog.Content>
          ...
          <form onSubmit={distributorForm.handleSubmit(handleAddDistributor)}>
            <Flex direction="column" gap="3">
              <Controller name="name" control={distributorForm.control} render={({ field }) => (
              ...
              <Controller name="region" control={distributorForm.control} render={({ field }) => (
              ...
              <Flex gap="3" justify="end">
                <Button type="submit" disabled={distributorForm.formState.isSubmitting}>{t("actions.save-distributor")}</Button>
              </Flex>
            </Flex>
          </form>
        </Dialog.Content>
      ...
      <Dialog.Root onOpenChange={setIsRegionModalOpen} open={isRegionModalOpen}>
        <Dialog.Content>
          ...
          <form onSubmit={regionForm.handleSubmit(handleAddRegion)}>
            <Controller name="name" control={regionForm.control} render={({ field }) => (
            ...
            <Button type="submit" disabled={regionForm.formState.isSubmitting}>{t("actions.save-region")}</Button>
          </form>
        </Dialog.Content>
      </Dialog.Root>
      ...
      <Dialog.Root onOpenChange={setIsComplianceReportModalOpen} open={isComplianceReportModalOpen}>
        <Dialog.Content>
          ...
          <form onSubmit={reportForm.handleSubmit(handleGenerateReport)}>
            <Controller name="reportType" control={reportForm.control} render={({ field }) => (
              <Select.Root value={field.value} onValueChange={field.onChange}>
                <Select.Trigger />
                <Select.Content>
            ...
            <Controller name="startDate" control={reportForm.control} render={({ field }) => (
              <TextField.Root type="date" {...field}><TextField.Slot>Start Date</TextField.Slot></TextField.Root>
            )}/>

            <Controller name="endDate" control={reportForm.control} render={({ field }) => (
              <TextField.Root type="date" {...field}><TextField.Slot>End Date</TextField.Slot></TextField.Root>
            )}/>
            {reportForm.formState.errors.endDate && (
              <Text color="red" size="1">{reportForm.formState.errors.endDate.message}</Text>
            )}
            <Button type="submit" disabled={reportForm.formState.isSubmitting}>{t("actions.generate-report")}</Button>
          </form>
        </Dialog.Content>
      </Dialog.Root>
        ...
        <Table.Body>
          {distributors.map((distributor) => (
            <Table.Row key={distributor.id}>
              ...
              <Table.Cell>
                <Badge variant="soft" color="green">{t(`compliance-status.${distributor.compliance}`)}</Badge>
              </Table.Cell>
              <Table.Cell>{distributor.onTimeDelivery}%</Table.Cell>
              <Table.Cell>
                <Badge color={distributor.licenses === "active" ? "green" : "red"}>{t(`license-status.${distributor.licenses}`)}</Badge>
              </Table.Cell>
